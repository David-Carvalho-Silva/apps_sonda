...
def obter_intervalo_datas():
    while True:
        data_inicio_str = input("Digite a data de início (formato DD/MM/YYYY): ")
        data_fim_str = input("Digite a data de fim (formato DD/MM/YYYY): ")
        try:
            data_inicio = datetime.datetime.strptime(data_inicio_str, '%d/%m/%Y')
            data_fim = datetime.datetime.strptime(data_fim_str, '%d/%m/%Y')
            if data_inicio > data_fim:
                print("A data de início deve ser anterior ou igual à data de fim.")
            else:
...
def obter_token():
    token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMzM1OjE5ODEzIiwiem5lIjo0LCJqdGkiOiJmZ0cwRlY5SElmMlBCYzJaIiwiaWF0IjoxNzE4NDExMjY5fQ.rUyJt_juqgl2n6ukHw_8OoOIJMpaqnX_L1f99O1Wrs8'
    return token
...
def processar_viagens(data_atual, token_autorizacao):
    ...
    with open('linha_ids.txt', 'w') as file:
        for id in linha_ids:
            file.write(f"{id}\n")

    linha_idhorario = list(set(entrada["horarioId"] for entrada in data_gestão_de_falhas))
    with open('id_horario.txt', 'w') as file:
        for id in linha_idhorario:
            file.write(f"{id}\n")
    ...
    for linha_id in linha_ids:
        ...
        try:
            ...
            with open('dashboard_response.txt', 'w') as file:
...
def main():
    token_autorizacao = obter_token()
    intervalo_datas = None
    resultados_acumulados = []

    while True:
        if intervalo_datas is None:
        ...
        df_horarios_nao_batem_condicao.to_excel("horarios_nao_batem_condicao.xlsx", index=False, engine='openpyxl')
        
        print("\nEscolha uma opção:")
        print("1. Verificar outro intervalo de datas")
        print("2. Verificar novamente o mesmo intervalo")
